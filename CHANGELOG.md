...
> 优化 115 离线提交体验并收紧在线播放临时数据：交互式离线改为持久后台队列，在线播放按正式整理逻辑清理无用文件，同时修复文件大小和本机播放器选择。

### ⚡ 115 离线异步队列

- 作品详情点击“115 离线”后不再等待 115 网络调用；本地创建任务并写入 SQLite 队列后立即返回。
- 前端直接复用当前页面已经加载的作品和磁力数据，避免点击提交时再次请求 JavDB。
- 后台 worker 按 FIFO 消费队列，并在真正提交前原子 claim 为“正在提交 115”，避免重复消费。
- 115 响应再慢也不会阻塞交互请求线程；提交成功后进入 submitted，失败则转为 115_submit_failed。
- 队列持久化在 SQLite，应用重启后未提交任务仍可继续处理。
- pending 队列任务参与重复任务检测，避免用户快速连续点击产生重复任务。
- 保留现有提交未完成兜底恢复逻辑。
- Telegram、关注自动下载等原有后台链路继续保持既有行为，不被交互式队列改动影响。

### ▶️ 在线播放整理与显示

- 在线播放选定主视频后，临时目录按普通整理思路清理：保留选中的主视频和字幕，删除其它视频、sample、广告、说明文件等无用数据。
- 多主视频场景先由用户选择，再执行清理，避免提前删除正确文件。
- 修复在线播放弹窗文件大小单位错误：115 返回 bytes，前端现在直接按 bytes 转换为 KB / MB / GB。
- 播放器选项收敛为“本机播放器（系统选择）”和“浏览器播放”两项。
- 不再写死 PotPlayer / VLC；旧播放器偏好会迁移到系统选择。
- 本机播放器模式优先调用设备原生选择能力，不支持时退化为系统直接打开播放地址。

### ✅ 可靠性

- 增加在线播放无用文件清理、字幕保留、主视频选择和异步 115 队列回归测试。
- 队列任务在提交前进行原子状态领取，降低并发 worker 重复提交同一磁力的风险。
- Python 3.12 / 3.13、前端、移动端浏览器、Web E2E 和 Docker 安全/双架构构建门禁通过后发布。
- 无数据库结构迁移或新增必填配置。

### 📦 升级说明

- 镜像：`ghcr.io/huhengbo/javdb115:0.1.6`，同时更新 `0.1` 和 `latest`，支持 `linux/amd64` / `linux/arm64`。
- 保留现有数据库目录/卷和 `APP_SECRET_KEY`，拉取新镜像并重建容器即可。
- 已存在的任务与设置保持兼容；新的交互式离线队列直接复用现有 SQLite。
- PWA 用户升级服务端后刷新页面或使用应用内更新提示即可。

## [0.1.5] - 2026-09-25

> 修复 115 在线播放提交链路与等待反馈：确保磁力真实提交到 115，并将播放准备过程改为居中的进度弹框。

### ▶️ 在线播放修复

- 修复点击“在线播放”后可能长时间无响应、115 中看不到离线任务的问题。
- 不再依赖 115 `savepath` 参数创建播放目录；改为先显式创建 `.play/<session>` 目录，再将磁力直接提交到该目录。
- 播放等待阶段持续读取 115 离线进度；在视频文件尚未出现前保持“115 离线中”状态，避免过早进入文件定位阶段。
- 视频文件出现后再进行主视频识别和播放地址解析，进度阶段更符合真实状态。
- 清理逻辑始终删除播放 session 根目录，并继续同步清理对应的 115 离线任务记录。

### 🎨 播放交互

- 播放准备界面改为屏幕中心模态弹框，不再使用底部 Sheet。
- 新增圆形百分比进度、横向进度条和当前阶段提示。
- 多文件场景在同一弹框中选择视频；播放地址准备完成后原位切换为播放方式和“立即播放”。
- 支持系统/浏览器、PotPlayer、VLC 和复制播放地址，延续已有播放方式记忆。

### ✅ 可靠性

- 增加显式 session 目录作为 115 离线目标的回归验证。
- Python 3.12 / 3.13、前端、移动端浏览器、Web E2E 和 Docker 安全/双架构构建门禁均已通过。
- 无数据库迁移或配置变更。

### 📦 升级说明

- 镜像：`ghcr.io/huhengbo/javdb115:0.1.5`，同时更新 `0.1` 和 `latest`，支持 `linux/amd64` / `linux/arm64`。
- 保留现有数据库目录/卷和 `APP_SECRET_KEY`，拉取新镜像并重建容器即可。
- PWA 用户升级服务端后刷新页面或使用应用内更新提示即可。

## [0.1.4] - 2026-09-24

> 新增 115 临时在线播放能力：磁力可直接临时离线、自动定位主视频并调用本机播放器，同时完善临时文件与 115 离线记录清理。

### ▶️ 在线播放

- 作品详情磁力操作新增“在线播放”，与现有“115 离线 / 复制 / 打开”并列，不进入正式下载整理任务。
- 在线播放使用 115 下载临时目录下独立的 `.play/<session>` 目录，按播放会话隔离临时资源。
- 播放任务通过前端轮询推进：提交 115 离线后自动等待目录出现、扫描视频文件并获取播放直链，无需额外后台 worker。
- 自动过滤 sample、preview、trailer、广告等明显无关视频，并优先选择体积最大的主视频；多个大文件体积接近时由用户选择。
- 支持系统/浏览器打开、PotPlayer、VLC，以及复制播放地址；默认播放方式保存在浏览器本地。
- 播放地址由服务端通过现有 115 Cookie 获取，115 凭据不会暴露给前端。

### 🧹 临时资源清理

- 播放 session 默认保留 6 小时，过期后清理对应临时资源。
- 清理时先删除 115 离线任务记录，再删除确认属于 `.play` 的临时目录，避免相同磁力后续重新播放时被旧离线记录阻塞。
- 删除 115 离线记录使用仅删除任务记录的模式，不让 115 API 顺带删除文件；临时目录仍由应用自己的目录归属检查负责删除。
- 清理操作只作用于在线播放专用目录，不影响正式离线下载、整理目录和已有媒体文件。

### ✅ 可靠性

- 新增 115 播放直链、主视频自动选择、多文件选择以及离线记录清理回归测试。
- Python 3.12 / 3.13 的 Ruff、Mypy、编译与测试通过；前端 lint/test/build 和 Web E2E smoke 通过。
- 无数据库结构迁移要求，现有任务和设置保持兼容。

### 📦 升级说明

- 镜像：`ghcr.io/huhengbo/javdb115:0.1.4`，同时更新 `0.1` 和 `latest`，支持 `linux/amd64` / `linux/arm64`。
- 保留现有数据库目录/卷和 `APP_SECRET_KEY`，拉取新镜像并重建容器即可。
- 在线播放依赖设置中已配置有效的 115 登录状态和下载临时目录。
- PWA 用户升级服务端后刷新页面或使用应用内更新提示即可。
